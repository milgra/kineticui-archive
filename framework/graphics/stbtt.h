...
//           stbtt_GetCodepoint_bitmap()           -- allocates and returns a bitmap
//           stbtt_MakeCodepoint_bitmap()          -- renders into bitmap you provide
//           stbtt_GetCodepoint_bitmapBox()        -- how big the bitmap must be
//
...
int main(int argc, char **argv)
{
   ...
   bitmap = stbtt_GetCodepoint_bitmap(&font, 0,stbtt_ScaleForPixelHeight(&font, s), c, &w, &h, 0,0);
...
int main(int arg, char **argv)
{
   ...
   while (text[ch]) {
      ...
      stbtt_GetCodepoint_bitmapBoxSubpixel(&font, text[ch], scale,scale,x_shift,0, &x0,&y0,&x1,&y1);
      stbtt_MakeCodepoint_bitmapSubpixel(&font, &screen[baseline + y0][(int) xpos + x0], x1-x0,y1-y0, 79, scale,scale,x_shift,0, text[ch]);
...
extern int stbtt_GetCodepoint_box(const stbtt_fontinfo *info, int codepoint, int *x0, int *y0, int *x1, int *y1);
...
extern unsigned char *stbtt_GetCodepoint_bitmap(const stbtt_fontinfo *info, float scale_x, float scale_y, int codepoint, int *width, int *height, int *xoff, int *yoff);
...
extern unsigned char *stbtt_GetCodepoint_bitmapSubpixel(const stbtt_fontinfo *info, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint, int *width, int *height, int *xoff, int *yoff);
// the same as stbtt_GetCodepoitnBitmap, but you can specify a subpixel
// shift for the character

extern void stbtt_MakeCodepoint_bitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, int codepoint);
// the same as stbtt_GetCodepoint_bitmap, but you pass in storage for the bitmap
// in the form of 'output', with row spacing of 'out_stride' bytes. the bitmap
// is clipped to out_w/out_h bytes. Call stbtt_GetCodepoint_bitmapBox to get the
// width and height and positioning info for it first.

extern void stbtt_MakeCodepoint_bitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint);
// same as stbtt_MakeCodepoint_bitmap, but you can specify a subpixel
// shift for the character

extern void stbtt_GetCodepoint_bitmapBox(const stbtt_fontinfo *font, int codepoint, float scale_x, float scale_y, int *ix0, int *iy0, int *ix1, int *iy1);
// get the bbox of the bitmap centered around the glyph origin; so the
// bitmap width is ix1-ix0, height is iy1-iy0, and location to place
// the bitmap top left is (leftSideBearing*scale,iy0).
// (Note that the bitmap uses y-increases-down, but the shape uses
// y-increases-up, so Codepoint_bitmapBox and Codepoint_box are inverted.)

extern void stbtt_GetCodepoint_bitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint, float scale_x, float scale_y, float shift_x, float shift_y, int *ix0, int *iy0, int *ix1, int *iy1);
// same as stbtt_GetCodepoint_bitmapBox, but you can specify a subpixel
// shift for the character
...
int stbtt_GetCodepoint_box(const stbtt_fontinfo *info, int codepoint, int *x0, int *y0, int *x1, int *y1)
{
...
//   printf( "\nglyphs %i : %i" , glyph1 , glyph2 );
//   printf( "\nneedle %u" , needle );
//   while (l <= r)
//   {
//      m = (l + r) >> 1; // get mathematical middle
//      straw = ttULONG(data+18+(m*6)); // note: unaligned read
//      printf( "\nl %i r %i m %i straw %u" , l , r , m , straw );
//      if (needle < straw)
//      {
//         r = m - 1;
//      }
...
//   }
   while ( l < r )
   {
        ...
        straw = ttULONG(data+18+(l*6)); // note: unaligned read
//        printf( "\nglyphs %i : %i kern : %i left : %i right %i straw %i needle %i" , ( straw >> 16 ) & 0xFF , straw & 0xFF , ttSHORT(data+22+(l*6)) , l , r , straw , needle );
//        printf( "\nl %i r %i straw %i value %i" , l , r , straw , ttSHORT(data+22+(l*6)) );
       
        if ( straw == needle )
        {
//            printf( "\n straw == needle %i" , ttSHORT(data+22+(l*6)) );
            return ttSHORT(data+22+(l*6));
        }
...
int  stbtt_GetCodepointKernAdvance(const stbtt_fontinfo *info, int ch1, int ch2)
{
   ...
   int result = stbtt_GetGlyphKernAdvance(info, stbtt_FindGlyphIndex(info,ch1), stbtt_FindGlyphIndex(info,ch2));
   return result;
}
...
void stbtt_GetCodepoint_bitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint, float scale_x, float scale_y, float shift_x, float shift_y, int *ix0, int *iy0, int *ix1, int *iy1)
{
   stbtt_GetGlyphBitmapBoxSubpixel(font, stbtt_FindGlyphIndex(font,codepoint), scale_x, scale_y,shift_x,shift_y, ix0,iy0,ix1,iy1);
}

void stbtt_GetCodepoint_bitmapBox(const stbtt_fontinfo *font, int codepoint, float scale_x, float scale_y, int *ix0, int *iy0, int *ix1, int *iy1)
{
   stbtt_GetCodepoint_bitmapBoxSubpixel(font, codepoint, scale_x, scale_y,0.0f,0.0f, ix0,iy0,ix1,iy1);
}
   ...
   while (e) {
      if (w == 0) {
         ...
         if (w == 0) {
            ...
            if (i < len && j >= 0) {
               if (i == j) {
                  // x0,x1 are the same pixel, so compute trafoC coverage
...
unsigned char *stbtt_GetCodepoint_bitmapSubpixel(const stbtt_fontinfo *info, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint, int *width, int *height, int *xoff, int *yoff)
{
   return stbtt_GetGlyphBitmapSubpixel(info, scale_x, scale_y,shift_x,shift_y, stbtt_FindGlyphIndex(info,codepoint), width,height,xoff,yoff);
}   

void stbtt_MakeCodepoint_bitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint)
{
   stbtt_MakeGlyphBitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, shift_x, shift_y, stbtt_FindGlyphIndex(info,codepoint));
}

unsigned char *stbtt_GetCodepoint_bitmap(const stbtt_fontinfo *info, float scale_x, float scale_y, int codepoint, int *width, int *height, int *xoff, int *yoff)
{
   return stbtt_GetCodepoint_bitmapSubpixel(info, scale_x, scale_y, 0.0f,0.0f, codepoint, width,height,xoff,yoff);
}   

void stbtt_MakeCodepoint_bitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, int codepoint)
{
   stbtt_MakeCodepoint_bitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, 0.0f,0.0f, codepoint);
}
